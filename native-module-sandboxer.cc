...
int main(int argc, char* argv[]) {
  ...
  absl::SetProgramUsageMessage(
      absl::StrFormat("A program to sandbox Veracruz native modules.\n"
  ...
  // XXX: Initializing the log takes a while. Let's disable that for now
  //absl::InitializeLog();
  ...
  if (!mounts_string.empty()) {
    for (absl::string_view mount : absl::StrSplit(mounts_string, ',')) {
      ...
      if ((st.st_mode & S_IFMT) == S_IFDIR) {
        ...
        builder.AddFileAt(source, target, false);
      }
    }
  }
